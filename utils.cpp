...
//CPMZ
//generate a random int from 0 to a-1;
int Utils::randomi(int a){
...
//generate a random int from 1 to a
int Utils::randomi2(int a)
{
 timeval t1;
 gettimeofday(&t1, NULL);
 srand(t1.tv_usec*t1.tv_sec);
...
int Utils::randomir(int a, int b){           //return of a int in [a,b] range
   timeval t1;
   gettimeofday(&t1, NULL);
   srand(t1.tv_usec*t1.tv_sec);

   int range = b-a+1;
   int randn = a + (rand() % range);
   return randn;
}
